...
/**
 * Questa classe è la rappresentazione astratta di una funzione. Ogni function
 * di una servlet application deve estendere questa classe ed implementare i
 * metodi astratti. Il metodo mostra() viene richiamato dalle GET; il metodo
 * elabora dalle POST.
 * <p>
 * Prima di richiamare il metodo mostra() o elabora() della sua sottoclasse
 ...
 * </ul>
 * <p>
 * Se checkAuthentication() fallisce viene richiamato il metodo della
 * sottoclasse checkAuthenticationFailed() o checkFieldAutFailed() a seconda del
 * tipo di errore incontrato.
 * <p>
 * Se checkStatusPermission fallisce viene richiamato il metodo
 * permissionFailed() che puo' essere ridefinito opportunamente dalle
 * sottoclassi.
 * <p>
 * Se checkRequestParameter() fallisce viene richiamato il metodo
 * checkFieldFailed() della sottoclasse.
 * <p>
 *
 */
public abstract class Function_base implements Function_itf {
	...
	/**
	 * La doget base verifica che l'applicazione e/o la funzione siano attive
	 * (Stato), che l'utente possa accedere alla funzione, la presenza e la validità
	 * dei campi della richiesta e l'autenticazione. Se tutto OK richiama il metodo
	 * mostra().
	 *
	 * @param req SsbServletRequest che incapsula la richiesta al servlet.
	 * @param res SsbServletResponse che incapsula la risposta dal servlet.
	...
	public final void doGet(SsbServletRequest req, SsbServletResponse res) throws AppCrash {
		...
		try {
			...
			if (_applicationSrv.isTraceEnabled()) {
				...
				if (getName().equals("InserimentoALIMOD20")) {
					log.append("#Tracef=").append(getName())
							.append(" - ANNEX11 - Inserimento nuovo PDA in bozza o visualizzazione PDA esistente");
					Logger.GetInstance().log0(log.toString());
				}
				if (getName().equals("RicercaALIMOD20")) {
					log.append("#Tracef=").append(getName()).append(" - ANNEX11 - Ricerca PDA");
					Logger.GetInstance().log0(log.toString());
				}
	...
	/**
	 * La doPost base verifica che l'applicazione e/o la funzione siano attive
	 * (Stato), che l'utente possa accedere alla funzione, la presenza e la validità
	 * dei campi della richiesta e l'autenticazione. Se tutto OK richiama il metodo
	 * elabora().
	 *
	...
	public final void doPost(SsbServletRequest req, SsbServletResponse res) throws AppCrash {
		...
		try {
			...
			if (_applicationSrv.isTraceEnabled()) {
				...
				log = new StringBuffer(100);
				if (getName().equals("ApprovalWorkFlow")) {
					log.append("#Tracef=").append(getName())
							.append(" - ANNEX11 - Invio in approvazione o approvazione del PDA creato");
					Logger.GetInstance().log0(log.toString());
				}
			}
	...
	/**
	 * Questo metodo di default non fa nulla. <b>Viene richiamato prima di eseguire
	 * qualsiasi controllo su autenticazione, permessi e parametri</b>. Permette,
	 * alle classi concrete che estendono questa classe, di effettuare operazioni
	 * propedeutiche all'elaborazione della get o post.
	 *
	 * @param req net.project.servlet.frame.SsbServletRequest oggetto che incapsula
	 *            la richiesta dal client alla servlet
	 * @param res net.project.servlet.frame.SsbServletResponse oggetto che incapsula
	 *            la risposta della servlet al client.
	 * @exception net.project.errors.AppCrash
	 */
	@Override
	...
	/**
	 * Controlla che la funzione sia attiva e che l'utente abbia i permessi di
	 * accesso.
	 *
	 * @param req SsbServletRequest che incapsula la richiesta al servlet.
	 * @param res SsbServletResponse che incapsula la risposta dal servlet.
	...
	/**
	 * Questo metodo viene richiamato dal framework se il controllo sui permessi di
	 * accesso alla funzione fallisce. Di default esegue la displayPage() di una
	 * pagina. La pagina si chiama 'PageError' e deve esistere nel file di
	 * configurazione. In questa pagina vengono settati due parametri: 'Name' con il
	 * functionID della funzione e 'Msg' con un messaggi del framework.
	 * <P>
	 * Le classi concrete che estendono function_base dovrebbero ridefinire questo
	 * metodo per dare alla applicazione un comportamento consono alle sue
	 * necssita'.
	 *
	 * @param req     SsbServletRequest che incapsula la richiesta al servlet.
	 * @param res     SsbServletResponse che incapsula la risposta dal servlet.
	...
	/**
	 * Esegue l'autenticazione utilizzando la classe che implementa
	 * AuthenticationProvider_itf associata alla fuction. Questa classe viene
	 * recuperata attraverso il framework.
	 *
	 * @param req      SsbServletRequest che incapsula la richiesta al servlet.
	 * @param res      SsbServletResponse che incapsula la risposta dal servlet.
	 * @param userInfo net.project.servlet.security.UserSecurityInfo.
	 * @exception net.project.errors.AppCrash. return true se l'autenticazione va a
	 ...
	 */
	  private boolean checkAuthentication(SsbServletRequest req, SsbServletResponse res, UserSecurityInfo userInfo)
	        ...
	        try {
	            ...
	            if (userInfo.getRoleId() == null || (userInfo.getUserStatus()!= null && userInfo.getUserStatus())) {
	                if (_applicationSrv.isTraceEnabled()) {
	...
	/**
	 * Controlla la presenza e la validità dei campi della richiesta. Per effettuare
	 * il compito richiama il metodo checkField() che deve essere implementato da
	 * tutte le sue sottoclassi.
	 * <p>
	 * Se abilitato il firewall parametri allora va a verificare per tutti i
	 * parametri della request se esiste nel vocabolario del messaggio HTTPREQUEST
	 * un verifier per quel campo. Se lo trova esegue la verifica, se non lo trova
	 ...
	 * I controlli firewall vengono effettuati prima della chiamata a checkField se
	 *
	 * @param req SsbServletRequest che incapsula la richiesta al servlet.
	 * @param res SsbServletResponse che incapsula la risposta dal servlet.
	 * @exception net.project.errors.AppCrash. return true se il controllo ha esito
	...
	private boolean checkRequestParameter(SsbServletRequest req, SsbServletResponse res) throws AppCrash {
		...
		// Una volta eseguiti i controlli applicativi se il check parametri e' server
		// side
	...
	/**
	 * Attiva una funzione. Questo metodo viene richiamato una sola volta alla
	 * partenza della servlet application
	 *
	 * @return void.
	 */
	@Override
	...
	/**
	 * Verifica la necessità di effettuare l'autenticazione. Di default restituisce
	 * true. Questo fa si che l'accesso alla funzione sia condizionato da una
	 * precedente autenticazione.
	 *
	...
	/**
	 * Controlla la presenza e la validità dei campi della richiesta. Ogni classe
	 * deve implementare questo metodo astratto per eseguire la verifica dei campi
	 * in ingresso
	 *
	 * @param req SsbServletRequest che incapsula la richiesta al servlet.
	...
	/**
	 * Esegue l'azione da compiere in caso di non corretta formattazione dei campi
	 * della richiesta.Deve essere implementato dalle classi concrete
	 *
	 * @param req SsbServletRequest che incapsula la richiesta al servlet.
	 * @param res SsbServletResponse che incapsula la risposta dal servlet.
	 * @exception net.project.errors.AppCrash.
	...
	/**
	 * Esegue l'azione da compiere in caso di non corretta formattazione dei campi
	 * necessari all'autenticazione.Deve essere implementato dalle classi concrete
	 *
	 * @param req SsbServletRequest che incapsula la richiesta al servlet.
	 * @param res SsbServletResponse che incapsula la risposta dal servlet.
	 * @exception net.project.errors.AppCrash.
	...
	/**
	 * Esegue l'azione da compiere in caso di fallimento dell'autenticazione.Deve
	...
	/**
	 * Metodo per richiamare la visualizzazione del messaggio di errore in caso di
	 * eccezioni nel framework. Ritorna true se il messaggio e' stato effettivamente
	 * visualizzato. Ritorna false se la visualizzazione deve essere demandata alla
	 * servlet application facendo la throw della eccezione.
	 */
	...
	/**
	 * Metodo per la gestione della visualizzazione del messaggio di errore in caso
	 * di eccezioni non gestite dalle implementazioni o generate nel framework
	 * stesso. Deve restituire true se ha effettivamente visualizzato il messaggio.
	 * Se restituisce false l'eccezione viene rilanciata verso la
	 * ServletApplication_base che la 'gira' al su metodo displayError.
	 * L'implementazione di default restituisce false
	 */
	...
	/**
	 * Questo metodo deve verificare la presenza della sessione, se necessaria. Se
	 * tutto ok deve ritornare true altrimenti false. L'implementazione di default
	 * ritorna sempre true. Le varie servlet application devono ridefinire questo
	 * metodo e fornire delle implementazioni adeguate. Per verificare semplicemente
	 * l'esistenza della sessione dalle sottoclassi si puo' richiamare il metodo
	 * simpleCheckSession(). La verifica della presenza della sessione dovrebbe
	 * includere il controllo dell'esistenza in sessione degli oggetti necessari.
	 * <p>
	 * Il framework richiama questo metodo subito dopo la prepareForRequest() e
	 * prima di checkAuthentication() e checkStatusPermission().
	 *
	 * @param req SsbServletRequest che incapsula la richiesta al servlet.
	...
	/**
	 * Questo metodo verifica semplicemente la presenza della sessione. Se presente
	 * ritornare true altrimenti false.
	 *
	 * @param req SsbServletRequest che incapsula la richiesta al servlet.
	 * @return boolean true ok, false sessione scaduta.
	 */
	...
	/**
	 * Questo metodo applica le regole firewall sui parametri. Se abilitato il
	 * firewall parametri allora va a verificare per tutti i parametri della request
	 * se esiste nel vocabolario del messaggio HTTPREQUEST un verifier per quel
	 * campo. Se lo trova esegue la verifica, se non lo trova usa il verifier del
	 * campo DEFAULTFIELD.
	 * <p>
	 * Il verifier per DEFAULTFIELD deve essere presente.
	 * <p>
	 * Il firewall parametri si attiva tramite la proprieta' Servlet.Firewall = true
	 * <p>
	 * Se la verifica di un campo e' ok il fw provvede anche all'escaping SQL del
	 * campo stesso.
	 *
	...
	/**
	 * Questo metodo verifica se l'escaping SQL e' permesso per questa funzione.
	 * Viene chiamato dal firewall parametri se abilitato
	 * <p>
	 * Di default ritorna true. Le funzioni concrete possono ridefinirlo per
	 * impedire l'escamping SOLO IN CASI ECCEZIONALI.
	 *
