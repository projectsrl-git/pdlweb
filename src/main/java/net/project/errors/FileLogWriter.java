...
public class FileLogWriter extends StreamLogWriter_base {
    ...
    protected void writeToStream(String message) {
        ...
        try {
            ErrDetector.GetInstance().preCond(getFileName()!=null,"log fileName is null"+getFileName());
            File logFile=new File(getFileName());
            ErrDetector.GetInstance().preCond(logFile.getParentFile()!=null,"log directory is null "+logFile.getParent());
            if (!logFile.getParentFile().isDirectory() || !logFile.getParentFile().exists()) {
