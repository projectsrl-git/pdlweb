 ...
 * 
 */
public class FunctionInserimentoRuoli extends FunctionInserimentoMenu {

	private static final String PAGE = "inserimento_ruoli";

	public FunctionInserimentoRuoli() {
		super();
		setPageMostra(PAGE);
		setPageElabora(PAGE);
	...
	public FunctionInserimentoRuoli(ApplicationServices_itf applServices, String functionID, String functionName) {
			...
			try {
				...
				while (dataSet.hasMoreElements()) {
					...
					if (dbRow != null) {
						...
						String strRUOLI = req.getField(dbCODICE + "_FLAG_" + dbORDINE);
						String dbRUOLI = (String) dbRow.getField(RuoliDAO.RUOLI);
						if (!strRUOLI.equals(dbRUOLI)) {
							flagIsModified=true;
							ruoliDAO.setField(RuoliDAO.RUOLI, strRUOLI);
						}
				...
				ac.logContext("FunctionInserimentoRuoli", "Errore nella ricerca campi del dataset " + dsName);
				throw ac;
			} finally {
				...
				if (dataSet != null) {
					try {
						...
						ac.logContext("FunctionInserimentoRuoli", "Errore nella close del dataset " + dsName);
					}
				}
			}
		}
