 ...
 * 
 */
public class FunctionRicercaImpostazioniFeriePermessi extends FunctionRicerca {

    private static final String PAGE_IMPOSTA = "ricerca_impostazioniferiepermessi";
    private static final String PAGE_RISULTATI = "browse_impostazioniferiepermessi";    
    

    public FunctionRicercaImpostazioniFeriePermessi() {
        super();
        setPageImposta(PAGE_IMPOSTA);
        setPageRisultati(PAGE_RISULTATI);
    }

    public FunctionRicercaImpostazioniFeriePermessi(ApplicationServices_itf applServices, String functionID, String functionName) {
	...
	protected HashMap prepareWhereCondition(SsbServletRequest req, HashMap<String, String> queryParameter) {
        ...
        if (!aziendaSessione.trim().equalsIgnoreCase("")) {	
    		str_composed_where_cond = str_composed_where_cond + " AND IMPOSTAZIONI_FERIEPERMESSI.AZIENDA IN (SELECT CODICE FROM GERARCHIA_AZIENDE WHERE GERARCHIA LIKE '%"+aziendaSessione+"%')";
        }

         
        if (!azienda.trim().equalsIgnoreCase("")) {	
        	str_composed_where_cond = str_composed_where_cond + " AND  IMPOSTAZIONI_FERIEPERMESSI.AZIENDA = '"+azienda+"'";
        }
