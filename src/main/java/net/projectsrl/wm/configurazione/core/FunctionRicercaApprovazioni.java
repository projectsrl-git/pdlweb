 ...
 * 
 */
public class FunctionRicercaApprovazioni extends FunctionRicerca {

    private static final String PAGE_IMPOSTA = "ricerca_approvazioni";
    private static final String PAGE_RISULTATI = "browse_approvazioni";    
    

    public FunctionRicercaApprovazioni() {
        super();
        setPageImposta(PAGE_IMPOSTA);
        setPageRisultati(PAGE_RISULTATI);
    }

    public FunctionRicercaApprovazioni(ApplicationServices_itf applServices, String functionID, String functionName) {
	...
	protected HashMap prepareWhereCondition(SsbServletRequest req, HashMap<String, String> queryParameter) {
        ...
        if (!azienda.trim().equalsIgnoreCase("")) {	
        	str_composed_where_cond = str_composed_where_cond + " AND APPROVAZIONI.AZIENDA = '"+azienda+"'";
        }
