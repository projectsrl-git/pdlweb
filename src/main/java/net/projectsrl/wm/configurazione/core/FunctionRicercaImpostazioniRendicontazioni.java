 ...
 * 
 */
public class FunctionRicercaImpostazioniRendicontazioni extends FunctionRicerca {

    private static final String PAGE_IMPOSTA = "ricerca_impostazionirendicontazioni";
    private static final String PAGE_RISULTATI = "browse_impostazionirendicontazioni";    
    

    public FunctionRicercaImpostazioniRendicontazioni() {
        super();
        setPageImposta(PAGE_IMPOSTA);
        setPageRisultati(PAGE_RISULTATI);
    }

    public FunctionRicercaImpostazioniRendicontazioni(ApplicationServices_itf applServices, String functionID, String functionName) {
	...
	protected HashMap prepareWhereCondition(SsbServletRequest req, HashMap<String, String> queryParameter) {
        ...
        if (!aziendaSessione.trim().equalsIgnoreCase("")) {	
    		str_composed_where_cond = str_composed_where_cond + " AND IMPOSTAZIONI_RENDICONTAZIONI.AZIENDA IN (SELECT CODICE FROM GERARCHIA_AZIENDE WHERE GERARCHIA LIKE '%"+aziendaSessione+"%')";
        }

         
        if (!azienda.trim().equalsIgnoreCase("")) {	
        	str_composed_where_cond = str_composed_where_cond + " AND  IMPOSTAZIONI_RENDICONTAZIONI.AZIENDA = '"+azienda+"'";
        }
