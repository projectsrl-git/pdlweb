 ...
 * 
 */
public class FunctionRicercaRichiedenti extends FunctionRicerca {

    private static final String PAGE_IMPOSTA = "ricerca_richiedenti";
    private static final String PAGE_RISULTATI = "browse_richiedenti";    
    

    public FunctionRicercaRichiedenti() {
        super();
        setPageImposta(PAGE_IMPOSTA);
        setPageRisultati(PAGE_RISULTATI);
    }

    public FunctionRicercaRichiedenti(ApplicationServices_itf applServices, String functionID, String functionName) {
	...
	protected HashMap prepareWhereCondition(SsbServletRequest req, HashMap<String, String> queryParameter) {
        ...
        if (!azienda.trim().equalsIgnoreCase("")) {	
        	str_composed_where_cond = str_composed_where_cond + " AND RICHIEDENTI.AZIENDA = '"+azienda+"'";
        }
