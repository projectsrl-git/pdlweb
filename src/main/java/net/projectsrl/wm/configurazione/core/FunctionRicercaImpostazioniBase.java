 ...
 * 
 */
public class FunctionRicercaImpostazioniBase extends FunctionRicerca {

    private static final String PAGE_IMPOSTA = "ricerca_impostazionibase";
    private static final String PAGE_RISULTATI = "browse_impostazionibase";    
    

    public FunctionRicercaImpostazioniBase() {
        super();
        setPageImposta(PAGE_IMPOSTA);
        setPageRisultati(PAGE_RISULTATI);
    }

    public FunctionRicercaImpostazioniBase(ApplicationServices_itf applServices, String functionID, String functionName) {
	...
	protected HashMap prepareWhereCondition(SsbServletRequest req, HashMap<String, String> queryParameter) {
        ...
        if (!aziendaSessione.trim().equalsIgnoreCase("")) {	
    		str_composed_where_cond = str_composed_where_cond + " AND IMPOSTAZIONI_BASE.AZIENDA IN (SELECT CODICE FROM GERARCHIA_AZIENDE WHERE GERARCHIA LIKE '%"+aziendaSessione+"%')";
        }

         
        if (!azienda.trim().equalsIgnoreCase("")) {	
        	str_composed_where_cond = str_composed_where_cond + " AND  IMPOSTAZIONI_BASE.AZIENDA = '"+azienda+"'";
        }
