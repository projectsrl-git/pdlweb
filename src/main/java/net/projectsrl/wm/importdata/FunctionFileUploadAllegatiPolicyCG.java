...
public class FunctionFileUploadAllegatiPolicyCG extends FunctionFileUploadDocumentiParametri_base {

    public FunctionFileUploadAllegatiPolicyCG() {

        super();
    }

    public FunctionFileUploadAllegatiPolicyCG(ApplicationServices_itf applServices, String functionID, String functionName) {
    ...
    protected String getPaginaChiusuraPopup() {
        ...
        return "file_upload_allegati_policy_chiudi_cg";
    }
    ...
    protected void loadData(String sessionUser, String absolutePathFileName, SsbServletRequest req) throws AppCrash {
        ...
        if (idTaggancio != null) {
            String inserisciNome = "INSERT INTO DOCAZIENDA_ATCH (ID_DOCAZIENDA,ID_DOCAZIENDA_ATCH,PERCORSO,FILENAME,AZIENDA,MATRICOLA,DATA_CARICAMENTO,ORA_CARICAMENTO) VALUES ('" + idTaggancio
                    + "','" + Utils.getUnique() + "','','"+fileName + "','"+azienda+"','T','"+Utils.getStringDataOggiRibaltata()+"','"+Utils.getOrario()+"')";
    ...
    protected HashMap<String, Object> setCommonTags(SsbServletRequest req, UserSecurityInfo userInfo) throws AppCrash {
        ...
        String aziendaTaggancio = (String) req.getSession(false).getAttribute(Costanti_itf.DOCAZIENDA_SESSIONE);
        if (idTaggancio != null) {
            templateData.put("ID_DOCAZIENDA", idTaggancio);
            templateData.put("AZIENDA", aziendaTaggancio);
        }
