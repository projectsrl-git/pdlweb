 ...
 * 
 */
public class FunctionInserimentoCurriculumCG extends FunctionInserimento {

	private static final String PAGE = "inserimento_curriculum_cg";
	...
	public FunctionInserimentoCurriculumCG() {
		super();
		setPageMostra(PAGE);
		setPageElabora(PAGE);
	...
	public FunctionInserimentoCurriculumCG(ApplicationServices_itf applServices, String functionID, String functionName) {
	...
	/**
	 * I campi contenenti i dati deiprospect mi arrivano con CODICE + RAGIONE
	 * SOCIALE in questo formato "F:IDANAAZIRIC=000000001;F:ANAAZIRIC=PROJECT
	 * SRL DI CAVA MANARA"
	 * 
	 * Devo separare i valori con apposito metodo di
	 * Utils.leggeStringaElencoCampi(<nome campo>)
	 */
	@SuppressWarnings("unchecked")
	 ...
	 @SuppressWarnings("unchecked")
		public void elabora(SsbServletRequest req, SsbServletResponse res, UserSecurityInfo userInfo) throws AppCrash {
	        ...
	        if(req.getField("OPZIONE_INSERIMENTO_MODIFICA").equals(OPZIONE_INSERIMENTO)){
	        	...
	        	String sqlSistemaCodiceCV="update curricul set codice = '"+codice1+progressivo+codice3+"' where taggancio = (select taggancio from"
	        ...
	        if (!templateData.get("IMP_BASE_DIP").equals("1")){
	        	...
	        	String sqlDeleteAssociazioneCommessa = "DELETE FROM COMMESSA_X_DIPENDENTE WHERE ID_DIPENDENTE='"+idDipendente+"'";
		        ...
		        String sqlInsertAssociazioneCommessa = "INSERT INTO COMMESSA_X_DIPENDENTE (ID_COMDIP,ID_COMMESSA,ID_DIPENDENTE,PREDEFINITA) VALUES ('"+idComdip+"','00000000000000000000','"+idDipendente+"','S')";
	        ...
	        if (!req.getField("AZIENDA_CV").equals("")){
	        	...
	        	if (nProfili.equals("0")){
	        		String queryInsert1="INSERT INTO PROFILO_X_DIPENDENTE (DAGGANCIO, ID_PROXDIP,ID_PRODIP,ID_PROFILO,ID_DIPENDENTE,PREDEFINITO) "
	        				+ "VALUES ('"+req.getField("ID_DIPENDENTE")+"','"+Utils.getUnique()+"','"+idToInsert+req.getField("ID_DIPENDENTE")+"','"+idToInsert+"','"+req.getField("ID_DIPENDENTE")+"','S')";
					net.projectsrl.wm.utils.WMUtils.executeQuery(queryInsert1);
	        	}
	        }else{
	        	String sqlDeleteProfiliNonUsati="DELETE FROM PROFILO_X_DIPENDENTE WHERE DAGGANCIO = '"+req.getField("ID_DIPENDENTE")+"'";
	        ...
	        if (req.getField("ASSOCIAZIONE").equals("PROFILO_ORARIO") && req.getField("PREDEFINITO").equals("S")){
				...
				String queryUpdate9="UPDATE PROFILO_X_DIPENDENTE SET PREDEFINITO='S' WHERE ID_DIPENDENTE='"+req.getField("ID_DIPENDENTE")+"' AND ID_PROFILO='"+req.getField("ID_PROFILO")+"'";
		    ...
		    if (req.getField("TIPO").equals("PROFILO_ORARIO")){
		    	...
		    	sqlUpdate2 ="UPDATE PROFILO_X_DIPENDENTE SET PREDEFINITO = 'N' WHERE ID_DIPENDENTE='"+idToUpdate+"' AND ID_PROXDIP<>'"+idUnivocoProfilo+"'";
	        ...
	        String dirMATRICOLA = _applicationSrv.getRoot()+"areadocumenti/"+req.getField("AZIENDA_CV")+"/"+req.getField("MATRICOLA")+"/";
	        ...
	        if (templateData.get("ESISTE").equals("NO")){
		        String new_pwd = "";
		        ...
		        String nominativo = req.getField("COGNOME")+" "+req.getField("NOME");
		        ...
		        if (esiste.equals("S")) {
		        	String sqlUpdateUtente = "UPDATE UTENTI SET USERID='"+destinatari_mail+"', EMAIL='"+destinatari_mail+"' WHERE ID_DIPENDENTE='"+idDipendente+"'";
			        net.projectsrl.wm.utils.WMUtils.executeQuery(sqlUpdateUtente);
			        templateData = inviaMail(req, templateData, from, new_pwd, destinatari_mail, nominativo, azienda, aziendaSalvata, esiste);
			        idUnivoco=getIdUnivocoUtente(idDipendente);
		        }else{
		        		if (azienda.equals("")){
		        			String sqlUpdateUtente = "UPDATE UTENTI SET USERID='"+req.getField("EMAIL")+"', AZIENDA='"+req.getField("AZIENDA_CV")+"', EMAIL='"+req.getField("EMAIL")+"', NOME='"+req.getField("NOME")+"', COGNOME='"+req.getField("COGNOME")+"' WHERE ID_DIPENDENTE='"+req.getField("TAGGANCIO")+"'";
		    		        net.projectsrl.wm.utils.WMUtils.executeQuery(sqlUpdateUtente);
		        		}
			    		...
			    		" VALUES ('"+destinatari_mail+"','"+new_pwd+"','D','"+idUnivoco+"','"+req.getField("NOME")+"','"+req.getField("COGNOME")+"','"+destinatari_mail+"','"+Utils.getStringDataOggiRibaltata()+"','2999/12/31','S','"+azienda+"','"+idDipendente+"','S')";
				        net.projectsrl.wm.utils.WMUtils.executeQuery(sqlInsertUtente);
				        String sqlUpdateDipendente = "UPDATE CURRICUL SET ID_UTENTE='"+idUnivoco+"' WHERE TAGGANCIO='"+idDipendente+"'";
	        	...
	        	String sqlUpdateUtente3 = "UPDATE UTENTI SET USERID='"+req.getField("EMAIL")+"', AZIENDA='"+req.getField("AZIENDA_CV")+"', EMAIL='"+req.getField("EMAIL")+"', NOME='"+req.getField("NOME")+"', COGNOME='"+req.getField("COGNOME")+"' WHERE ID_DIPENDENTE='"+req.getField("TAGGANCIO")+"'";
		 ...
		 @SuppressWarnings({ "unchecked" })
		    private HashMap inviaMail(SsbServletRequest req, HashMap templateData, String from, String new_pwd, String destinatari_mail, String nominativo, String azienda, String aziendaSalvata, String esiste) throws AppCrash {
		        ...
		        if (esiste.equals("S") && !aziendaSalvata.equals("")){
		        	oggetto="Aggiornamento utente DAFNE";
		        	corpo += "E' stato aggiornato il suo profilo utente per l'accesso a <a href='"+portale+"'>"+portale+"</a>, qui di seguito trova le credenziali per l'accesso al portale:\n" +
		    		"/n";
				    corpo += "<i>Userid</i>: " + destinatari_mail + "\n/n";
				    corpo += "La <i>Password</i> e' rimasta invariata\n/n";
				    corpo += "\n/nAcceda al sistema utilizzando il nuovo utente.\n\n/n/nCordiali Saluti\n/n<i>Il Team DAFNE</i>";
		        }else{
