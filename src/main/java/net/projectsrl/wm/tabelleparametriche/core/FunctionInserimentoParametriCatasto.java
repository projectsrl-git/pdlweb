 ...
 * 
 */
public class FunctionInserimentoParametriCatasto extends FunctionInserimento {

	private static final String PAGE = "inserimento_parametri";

	public FunctionInserimentoParametriCatasto() {
		super();
		setPageMostra(PAGE);
		setPageElabora(PAGE);
		setDatasetTestata("DataSetParametriCatasto");
	}

	public FunctionInserimentoParametriCatasto(ApplicationServices_itf applServices, String functionID, String functionName) {
	...
	protected HashMap loadVar(HashMap templateData, SsbServletRequest req) throws AppCrash {
		...
		if (option == null || option.equals("") || option.equals(OPZIONE_INSERIMENTO)) {
			...
			templateData.put("SUFFISSO_PARAMETRO", req.getField("CODICE").substring(3));
			putIdParametroInSession(req, templateData);
		}
		putIdParametroInSession(req, templateData);
	...
	@SuppressWarnings("unchecked")
    private void putIdParametroInSession(SsbServletRequest req, HashMap templateData) {
        ...
        req.getSession(false).setAttribute(Costanti_itf.ID_PARAMETRO_SESSIONE, idParametro);   
    ...
    public void mostra(SsbServletRequest req, SsbServletResponse res, UserSecurityInfo userInfo) throws AppCrash {
        ...
        if(req.getField("CODICE").startsWith("APP") || req.getField("PREFISSO_PARAMETRO").equals("APP")){
        	...
        	if (azienda.equals("")){
        		azienda=(String) req.getSession(false).getAttribute("AZIENDA_SESSIONE");
        	}
			...
			try {
				...
				while (dataSet.hasMoreElements()) {
					...
					if (dbRow != null) {
						String esiste=getEsisteRiga(azienda,(String) dbRow.getField("CODICE"));
						if(!esiste.equals("S")){
							String sqlInsertGruppi ="INSERT INTO PARA (CODICE,DESCRI,AZIENDA,GR_AUTORIZZAZ) VALUES ('APP"+Utils.getUnique()+"','Approvatore','"+azienda+"','"+(String) dbRow.getField("CODICE")+"')";
							net.projectsrl.wm.utils.WMUtils.executeQuery(sqlInsertGruppi);
						}
				...
				ac.logContext("FunctionInserimentoParametriCatasto", "Errore nella ricerca campi del dataset " + dsName);
				throw ac;
			} finally {
				if (dataSet != null) {
					try {
						...
						ac.logContext("FunctionInserimentoParametriCatasto", "Errore nella close del dataset " + dsName);
					}
				}
			}
        }
	...
	private String getEsisteRiga(String azienda, String codice) throws AppCrash {
        ...
        try {
            ...
            params.put("AZIENDA_TENDINA", azienda+"' AND GR_AUTORIZZAZ='"+codice);
