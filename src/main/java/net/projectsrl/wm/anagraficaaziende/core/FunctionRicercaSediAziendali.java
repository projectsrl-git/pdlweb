 ...
 * 
 */
public class FunctionRicercaSediAziendali extends FunctionRicerca {

    private static final String PAGE_IMPOSTA = "ricerca_sediaziendali";
    private static final String PAGE_RISULTATI = "browse_sediaziendali";    
    

    public FunctionRicercaSediAziendali() {
        super();
        setPageImposta(PAGE_IMPOSTA);
        setPageRisultati(PAGE_RISULTATI);
    }

    public FunctionRicercaSediAziendali(ApplicationServices_itf applServices, String functionID, String functionName) {
	...
	protected HashMap prepareWhereCondition(SsbServletRequest req, HashMap<String, String> queryParameter) {
        ...
        if (!codice.trim().equalsIgnoreCase("")) {	
        	str_composed_where_cond = str_composed_where_cond + " AND SEDI_AZIENDALI.CODICE like '%"+codice.toUpperCase()+"'";
        }
        
        if (!ragSociale.trim().equalsIgnoreCase("")) {	
        	str_composed_where_cond = str_composed_where_cond + " AND SEDI_AZIENDALI.RAGSOC like '%"+ragSociale.toUpperCase()+"%'";
        }
