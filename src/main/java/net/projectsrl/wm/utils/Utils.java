...
public class Utils {
    ...
    public static String getStringDataOggiTrattino() {
    ...
    public static void creaMappa(UserSecurityInfo userInfo, Map<String, Object> templateData, String whereCondition, String percorsoBase, String whereConditionSito, String dataSetRicerca, Boolean test, String idImpianto) {
		String codImpianto= "";
    	...
    	try {
            ...
            //dataSet = dsFactory.makeDataSet("", "DSSitiPDL");
    		dataSet = dsFactory.makeDataSet("", "DSImpianti");
            HashMap<String, String> params = new HashMap<String, String>();
            params.put("WHERECONDITION_AZIENDE", whereCondition + " AND ID_IMPIANTO="+idImpianto);
            params.put("WHERECONDITION", "WHERE 1=1 ");
            dataSet.setParam(params);
            dataSet.open();
            while (dataSet.hasMoreElements()) {
            	Row_itf dbRow = (Row_itf) dataSet.nextElement();
            	codImpianto=dbRow.getField("ID_AZIENDA").toString();
            	fileNamePlanimetria=(String) dbRow.getField("FILENAME_PLANIMETRIA");
            	
            	sourceDir = percorsoBase + "/img/"+fileNamePlanimetria+".pdf";
    			semiDir = percorsoBase + "/img/"+fileNamePlanimetria+"_temp.pdf";
    			destinationDir = percorsoBase + "/img/";
    			...
    			if (sourceFile.exists()) {
    		        ...
    		        FileOutputStream out = new FileOutputStream(semiDir);
    		        PdfReader reader = new PdfReader(sourceDir);
    	            ...
    	            coloraInterferenzeImpianto(cb,userInfo,templateData, whereCondition, whereConditionSito, dataSetRicerca,idImpianto);
    	            
    	            cb.fill();
    	            cb.restoreState();
    	            
    	            testoInterferenzeImpianto(cb,bf,templateData,userInfo, whereCondition, whereConditionSito, dataSetRicerca, test,idImpianto);
    				...
    				PDDocument documentOut = PDDocument.load(semiDir);
    				@SuppressWarnings("unchecked")
    				...
    				documentOut.close();
    				System.out.println("Image "+fileName+" saved at -> " + destinationFile.getAbsolutePath());
    				templateData.put("IMMAGINE",templateData.get("IMMAGINE")+"<img src='img/"+fileNamePlanimetria+".jpg' class='img-responsive'>");
    				
    				File file = new File(semiDir);
    ...
    public static void creaMappaStampa(UserSecurityInfo userInfo, Map<String, Object> templateData, String whereCondition, String percorsoBase, String whereConditionSito, String dataSetRicerca, Boolean test, SsbServletRequest req, String idImpianto) {
		String fileNamePlanimetria= "";
		String codImpianto="";
    	String sourceDir = "";
    	String semiDir = "";
    	String destinationDir = "";
    	String descrImpianto="";
    	...
    	try {
            ...
            while (dataSet.hasMoreElements()) {
            	Row_itf dbRow = (Row_itf) dataSet.nextElement();
            	codImpianto=dbRow.getField("ID_IMPIANTO").toString();
            	descrImpianto=dbRow.getField("DESCR_IMPIANTO").toString();
            	fileNamePlanimetria=(String) dbRow.getField("FILENAME_PLANIMETRIA");
                
            	sourceDir = percorsoBase + "/img/"+fileNamePlanimetria+".pdf";
    			semiDir = percorsoBase + "/img/"+fileNamePlanimetria+"_"+Utils.getStringDataOggiTrattino()+"_"+req.getSession(false).getAttribute("CODICE_TURNO_SELEZIONATO")+"_"+descrImpianto+".pdf";
    			destinationDir = percorsoBase + "/img/";
    			File sourceFile = new File(sourceDir);
    			File destinationFile = new File(destinationDir);
    			if (!destinationFile.exists()) {
    			...
    			if (sourceFile.exists()) {
    				...
    				// modifico il pdf scrivendo nelle aree interessate
    	            ...
    	            whereConditionSito="AND ID_IMPIANTO="+codImpianto;
   	            	testoInterferenzeImpianto(cb,bf,templateData,userInfo, whereCondition, whereConditionSito, dataSetRicerca, test, idImpianto);
    ...
    private static void testoInterferenzeImpianto(PdfContentByte cb, BaseFont bf, Map<String, Object> templateData, UserSecurityInfo userInfo, String whereCondition, String whereConditionSito, String dataSetRicerca, Boolean test, String idImpianto) throws AppCrash {
    	int y=0;
		...
		DataSet_itf dataSet41 = null;
		DataSetFactory dsFactory41 = DataSetFactory.getInstance();
		dsFactory41 = DataSetFactory.getInstance();
        dataSet41 = dsFactory41.makeDataSet("", dataSetRicerca);
        HashMap<String, String> params = new HashMap<String, String>();
        params.put("WHERECONDITION_AZIENDE", whereCondition+ " AND ID_IMPIANTO="+idImpianto);
        params.put("WHERECONDITION", whereConditionSito);
        dataSet41.setParam(params);
        dataSet41.open();
        while (dataSet41.hasMoreElements()) {
        	Row_itf dbRow = (Row_itf) dataSet41.nextElement();
        	testoX=(int) dbRow.getField("TESTO_X");
        	testoY=(int) dbRow.getField("TESTO_Y");
        	aCapo=(int) dbRow.getField("A_CAPO");
        	
        	
        	if (test){
        		testo="0000-00 IMPRESA";
        	}else{
        		nrPDL=dbRow.getField("NR_PDL").toString();
            	impresa=dbRow.getField("IMPRESA_TESTO").toString();
        		testo=nrPDL+" "+impresa;
        	}
        	
        	if(testoX!=0 || testoY!=0 || aCapo!=0){
        		y=y+1;
 	            cb.moveText(testoX, testoY);
 	            cb.moveTextWithLeading(1, (float) (y*-aCapo));
            	cb.showText(testo);
        	}
    ...
    private static void coloraInterferenzeImpianto(PdfContentByte cb, UserSecurityInfo userInfo, Map<String, Object> templateData, String whereCondition, String whereConditionSito, String dataSetRicerca, String idImpianto) throws AppCrash {
        ...
        while (dataSet41.hasMoreElements()) {
        	Row_itf dbRow = (Row_itf) dataSet41.nextElement();
        	rectX=(int) dbRow.getField("RECT_X");
        	rectY=(int) dbRow.getField("RECT_Y");
        	rectW=(int) dbRow.getField("RECT_W");
        	rectH=(int) dbRow.getField("RECT_H");
