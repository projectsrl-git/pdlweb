 ...
 * 
 */
public class FunctionRicercaModulistica extends FunctionRicerca {

    private static final String PAGE_IMPOSTA = "ricerca_modulistica";
    private static final String PAGE_RISULTATI = "browse_modulistica";    
    

    public FunctionRicercaModulistica() {
        super();
        setPageImposta(PAGE_IMPOSTA);
        setPageRisultati(PAGE_RISULTATI);
    }

    public FunctionRicercaModulistica(ApplicationServices_itf applServices, String functionID, String functionName) {
	...
	protected HashMap prepareWhereCondition(SsbServletRequest req, HashMap<String, String> queryParameter) {
        ...
        String ruolo		 	= (String) req.getSession(false).getAttribute("AZIENDA_SESSIONE");
        if (ruolo.equals("D")){
        ...
        if (!azienda.trim().equalsIgnoreCase("")) {	
        	str_composed_where_cond = str_composed_where_cond + " AND DOCAZIENDA.AZIENDA = '"+azienda+"'";
        }
