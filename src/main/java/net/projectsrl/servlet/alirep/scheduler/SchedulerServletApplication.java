package net.projectsrl.servlet.alirep.scheduler;
...
public class SchedulerServletApplication extends BOWServletApplication {
	...
	private void makeScheduler(String taskListProp) {
	    ...
	    try {
			...
			ac.logContext(this.getClass().getName(), "erorre nella creazione della schedulazione per i task "+taskListProp);
		}
	}
	...
	private void readDBConfiguration() throws ParamCrash {
        ...
        ErrDetector.GetInstance().param(!dbUrl.equals(""),"in config.cfg non presente proprieta' DB.ConnectionURL");
	...
	public void destroy() {
		...
		try {
			...
			AppCrash ac = new AppCrash();
			ac.logContext(this.getClass().getName(), "erorre nello shutdown della schedulazione");
		}		
	}
