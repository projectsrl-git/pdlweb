...
public class DeferredMailSender {
                ...
                try {
                    ...
                    if (!serializableMail.isTimeToRetry()) {
                        ...
                        continue;
                         ...
                         if (serializableMail.toString().contains("#LINK_DOCS#")){
                    ...
                    if (serializableMail.getRetryAttempts() == 1) {
                    	if (serializableMail.toString().contains("#LINK_DOCS#")){
                    		 insertEmailLog(serializableMail, false, false);
                    	 }else{
                    	...
                    	if (serializableMail.toString().contains("#LINK_DOCS#")){
                    		updateEmailLog(serializableMail, false, false);
                   	 }else{
 ...
 // Aggiungi questo metodo pubblico nella classe DeferredMailSender

    public void updateAndOfferMail(UUID idMailLog, String newBody) throws AppCrash {
        
        EmailLogDAO daoMail = new EmailLogDAO();
        daoMail.setAttribute(EmailLogDAO.ID_EMAIL_LOG, idMailLog);
        
        if (!daoMail.retrieve()) {
            Logger.GetInstance().log0("EmailLog non trovato per ID: " + idMailLog);
            return;
        }

        try {
            // 1. Ottieni l'oggetto serializzato dal DB (EMAIL_DATA)
            String serializedData = (String) daoMail.getAttribute(EmailLogDAO.EMAIL_DATA);
            
            // 2. Deserializza l'oggetto SerializableMail
            SerializableMail serializableMail = deserializeObject(serializedData); 
            
            // 3. Aggiorna il corpo (BODY) all'interno dell'oggetto deserializzato
            serializableMail.setBody(newBody); 
            
            // 4. Re-serializza l'oggetto aggiornato
            String newSerializedData = serializeObject(serializableMail); 
            
            // 5. Aggiorna il DAO nel DB con il nuovo corpo e il nuovo oggetto serializzato
            daoMail.setAttribute(EmailLogDAO.BODY, newBody);
            daoMail.setAttribute(EmailLogDAO.EMAIL_DATA, newSerializedData);
            daoMail.setAttribute(EmailLogDAO.FL_SENT, false);
            daoMail.setAttribute(EmailLogDAO.FL_STOP_SEND, false);
            daoMail.update();
            
            // 6. Metti in coda l'oggetto SerializableMail aggiornato per l'invio immediato
            offer(serializableMail);
            
        } catch (Throwable th) {
            AppCrash ac = new AppCrash(th);
            ac.logContext(this.getClass().getName(), "Errore nell'aggiornamento e riaccodamento della mail " + idMailLog);
            throw ac;
        }
    }
}