...
public class InsertMultipleReferences<R extends NDAO_base> {
    ...
    public void insert(String[] keyArray, Integer keyId) throws AppCrash {

        try {
            for (String attributeId : keyArray) {
                ...
                try {
                    ...
                    ac.logContext(this.getClass().getName(), "attributeId:" + attributeId + " - keyId:" + keyId);
                }
            }

        } catch (Throwable e) {
            AppCrash ac = new AppCrash();
            ac.logContext(this.getClass().getName(), "foreingKeyFieldName:" + _foreingKeyFieldName + " - keyArray:"
                    + Arrays.toString(keyArray) + " - referenceFieldName:" + _referenceFieldName);
        }
    }

    protected void insert(Integer[] intKeyArray, Integer keyId) {

        try {
            for (Integer attributeId : intKeyArray) {
            ...
            AppCrash ac = new AppCrash();
            ac.logContext(this.getClass().getName(), "foreingKeyFieldName:" + _foreingKeyFieldName + " - intKeyArray:"
                    + Arrays.toString(intKeyArray) + " - referenceFieldName:" + _referenceFieldName);
        }
    }
