...
import net.projectsrl.webapp.authentication.LoginFailedCrash;
...
public abstract class FunctionProjectWebApp_base extends Function_base {
	...
	private static final String PAGE_SESSIONE_SCADUTA = "SessioneScaduta";
	...
	/**
	 * Verifica la necessit� di permessi per accedere alla funzione. Di default
	...
	protected String getPageName(SsbServletRequest req) {
	...
	protected void detectLastActivity(SsbServletRequest req, Map<String, Object> map) {
		...
		try {
			...
			pathDescri = getPathDescription(map);
			setSessionField(req, WebAppConstants_itf.SELECTED_MENU_ITEM, pathDescri);

			webappScope = getHostName(req.getRequestURL().toString());
			setSessionField(req, WebAppConstants_itf.APP_SCOPE, webappScope);
		} catch (Exception e) {
			AppCrash ac = new AppCrash(e);
			ac.logContext(this.getClass().getName(), "invalidate session on login page - functionId:" + functionId
					+ " - pathDescri:" + pathDescri + " - webappScope:" + webappScope);
	...
	private String getHostName(String url) {
		...
		try {
			...
			ac.logContext(this.getName(), "error retrieving host name - url:" + url);
		}
		return hostname;
	}
	...
	/**
	 * restituisce la chiave usata come id univoco del menu per tracciare attivit�
	 * corrente
	 * 
	...
	private void doMethod(SsbServletRequest req, SsbServletResponse res, UserSecurityInfo userInfo) throws AppCrash {
		...
		_applicationSrv.displayPage(getPageName(req), templateData,
				setPageDatasetParam(getPageName(), req, templateData), res);
	}

	private void addSessionMap(Map<String, Object> templateData, WebAppUserSecurityInfo<?> userInfo) {

		Map<String, Serializable> sessionMap = userInfo.getSessionMap();
	...
	protected boolean displayError(Throwable error, HttpServletRequest req, HttpServletResponse res) throws AppCrash {
		...
		if (error instanceof LoginFailedCrash) {
			page = PAGE_SESSIONE_SCADUTA;
		} else {
	...
	@SuppressWarnings("unchecked")
	protected Map<String, Object> getHistoryMap(SsbServletRequest req, UserSecurityInfo userInfo, String historyTag)
			throws AppCrash {

		Map<String, Object> templateData = null;

		if (req.getField(WebAppConstants_itf.FILTER_HISTORY) != null && req.getField(WebAppConstants_itf.FILTER_HISTORY).equals("Y")) {

			templateData = (Map<String, Object>) req.getSession(false).getAttribute(historyTag);
		...
		if (templateData != null) {
			Map<String, Object> sessionSearch = new HashMap<String, Object>();
			sessionSearch.putAll(templateData);
			req.getSession(false).setAttribute(historyTag, sessionSearch);
		} else {
			templateData = createMapFromRequest(req, userInfo);
		}
