...
public abstract class FunctionAjaxForm_base<D extends PjNDAO_base> extends FunctionProjectWebApp_base {
	...
	private D commitChanges(SsbServletRequest req, String operationType, UserSecurityInfo userInfo) throws AppCrash {
		...
		try {
			...
			} else if (WebAppConstants_itf.OPERATION_TYPE_RESUME.equals(operationType)) {
				resume(req, formDao, userInfo);

			} else if (WebAppConstants_itf.OPERATION_TYPE_CANCELLA.equals(operationType)) {
				cancella(req, formDao, userInfo);
			}
	...
	protected void resume(SsbServletRequest req, D formDao, UserSecurityInfo userInfo) throws AppCrash {
	...
	protected void cancella(SsbServletRequest req, D formDao, UserSecurityInfo userInfo) throws AppCrash {

		String keyField = req.getField("keyField");
		String keyValue = req.getField("keyValue");
		formDao.setAttribute(keyField, keyValue);
		formDao.cancella(userInfo);
	}
