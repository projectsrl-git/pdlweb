...
import net.projectsrl.qhse.moduli.alimod05.db.AliMod5DAO;
import net.projectsrl.qhse.moduli.core.FunctionInserimentoALIMOD_base;
import net.projectsrl.qhse.moduli.db.AliModDAO_base;

public class FunctionInserimentoALIMOD5 extends FunctionInserimentoALIMOD_base<AliMod5DAO> {

    public FunctionInserimentoALIMOD5(ApplicationServices_itf applServices, String functionID, String functionName) {
    ...
    public void mostra(SsbServletRequest req, SsbServletResponse res, UserSecurityInfo userInfo) throws AppCrash {
        ...
        if (isAnInsert(req)) {
            templateData.put(AliMod5DAO.ID_MODULO, "");

            templateData.put(AliMod5DAO.ID_UTENTE_CON, getSpecificUserInfo(userInfo).getIdUtente());
            templateData.put(AliMod5DAO.DT_MODULO, project.misc.Utils.getStringDataOggi());
            templateData.put(AliMod5DAO.STATO, StatiRichiesta.DRAFT.getCode());

        } else {
            PjNDAO_base rowToUpdate = new AliMod5DAO();
            String idRow = req.getField(AliMod5DAO.ID_MODULO);
            rowToUpdate.setAttribute(AliMod5DAO.ID_MODULO, idRow);
            ErrDetector.GetInstance().preCond(rowToUpdate.retrieve(), AliMod5DAO.ID_MODULO + " not found");
    ...
    @Override
    protected void sendResponseJSON(SsbServletResponse res, boolean result, String title, String message, AliMod5DAO formDao) {
    ...
    private String getFlagSitoAifa(AliMod5DAO formDao) throws AppCrash {
