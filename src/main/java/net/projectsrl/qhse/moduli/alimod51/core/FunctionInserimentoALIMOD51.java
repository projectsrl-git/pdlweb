...
import net.projectsrl.qhse.moduli.alimod51.db.AliMod51DAO;
import net.projectsrl.qhse.moduli.core.FunctionInserimentoALIMOD_base;
import net.projectsrl.qhse.moduli.db.AliModDAO_base;

public class FunctionInserimentoALIMOD51 extends FunctionInserimentoALIMOD_base<AliMod51DAO> {

    public FunctionInserimentoALIMOD51(ApplicationServices_itf applServices, String functionID, String functionName) {
    ...
    public void mostra(SsbServletRequest req, SsbServletResponse res, UserSecurityInfo userInfo) throws AppCrash {
        ...
        if (isAnInsert(req)) {
            templateData.put(AliMod51DAO.ID_MODULO, "");

            templateData.put(AliMod51DAO.ID_UTENTE_CON, getSpecificUserInfo(userInfo).getIdUtente());
            templateData.put(AliMod51DAO.DT_MODULO, project.misc.Utils.getStringDataOggi());
            templateData.put(AliMod51DAO.STATO, StatiRichiesta.DRAFT.getCode());

        } else {
            PjNDAO_base rowToUpdate = new AliMod51DAO();
            String idRow = req.getField(AliMod51DAO.ID_MODULO);
            rowToUpdate.setAttribute(AliMod51DAO.ID_MODULO, idRow);
            ErrDetector.GetInstance().preCond(rowToUpdate.retrieve(), AliMod51DAO.ID_MODULO + " not found");
    ...
    @Override
    protected void sendResponseJSON(SsbServletResponse res, boolean result, String title, String message, AliMod51DAO formDao) {
    ...
    private String getFlagSitoAifa(AliMod51DAO formDao) throws AppCrash {
