-- DROP TABLE "Tab_001 Anagrafica_PdL.FEB";
...
(
  ...
  "Altro_sez1_Prescrizioni_sic1_testo" character varying(50),
  ...
  "Protezioni auricolari" boolean DEFAULT false,
...
insert into temp_pdl select * from "Tab_001 Anagrafica_PdL.FEB";
...
delete from equipment where id_area in (select id_area from area_lavoro where id_impianto in (select id_impianto from impianti where id_azienda=(SELECT ID_AZIENDA from aziende where  codice ='FEB')));

delete from area_lavoro where id_impianto in (select id_impianto from impianti where id_azienda=(SELECT ID_AZIENDA from aziende where  codice ='FEB'));

delete from impianti where id_azienda=(SELECT ID_AZIENDA from aziende where  codice ='FEB');

-- impianti
insert into impianti (id_azienda,codice_impianto,descr_impianto,id_utente_ins)
select 
	(SELECT ID_AZIENDA from aziende where  codice ='FEB')
	,"Impianto"
...
SELECT 
	DISTINCT 
	id_impianto
	,"Area di Lavoro descr"
	,(select id_utente from utenti where username='assistenza')
FROM "Tab_008 Lista Equipment.FEB" as tle 
left outer join impianti as imp on tle."Impianto"=imp.codice_impianto and imp.id_azienda in (select id_azienda from aziende where codice ='FEB') 
where "Area di Lavoro descr" is not null
...
from (select distinct "Impianto","Area di Lavoro descr","Equipment","EIS"  from "Tab_008 Lista Equipment.FEB") as t1 
left outer join AREA_LAVORO as are on "Area di Lavoro descr"=descr_area and are.id_impianto in (select id_impianto from impianti where id_azienda in (select id_azienda from aziende where codice ='FEB') )
where "Equipment" is not null and are.id_area is not null
;

-- scarto queste equipment: 3
-- select * from "Tab_008 Lista Equipment.FEB" where "Equipment" is null or "Area di Lavoro descr" is null;


DELETE FROM PREPOSTI_IMPRESA WHERE ID_AZIENDA = (SELECT ID_AZIENDA from aziende where  codice ='FEB');

INSERT INTO PREPOSTI_IMPRESA (ID_AZIENDA,NOME_IMPRESA,NOME_COGNOME_PREPOSTO_IMPRESA)
SELECT 
DISTINCT
(SELECT ID_AZIENDA from aziende where  codice ='FEB') as ID_AZIENDA
,"Nome Azienda"
,"Riferimento Capo Cantiere" 
FROM "Tab_004 Ditte Terze.FEB"
...
select distinct 
	...
	,(select id_utente from utenti where username='assistenza')
from "Tab_001 Anagrafica_PdL.FEB" 
where "Impianto" not in (select codice_impianto from impianti where id_azienda in (select id_azienda from aziende where codice ='FEB'))
;	
...
select 
	...
	,true
	,(select id_utente from utenti where username='assistenza')
from "Tab_001 Anagrafica_PdL.FEB" as t1
left outer join impianti as imp on imp.codice_impianto = COALESCE(t1."Impianto",'ND') and imp.id_azienda in (select id_azienda from aziende where codice ='FEB')
...
DELETE FROM PDL WHERE ID_AZIENDA=(SELECT ID_AZIENDA from aziende where  codice ='FEB') and substring(dt_pdl,1,4)<>'2018';
--126996
...
SELECT 
	...
	ALTRO_SEZ1_PRESCRIZIONI_SIC2_TESTO
FROM TEMP_PDL as p1
left outer join impianti as imp on  coalesce(p1.IMPIANTO,'ND') = imp.codice_impianto and imp.id_impianto in (select id_impianto from impianti where id_azienda in (select id_azienda from aziende where codice ='FEB') )
