-- Table: "Tab_001 Anagrafica_PdL.CAS"
...
(
  ...
  "DMDS" boolean,
  "Oli" boolean,
  "Altro_sez1_Prodotti1_testo" character varying(255),
  "Rumore" boolean,
  ...
  "Altro_sez1_Attivita1_testo" character varying(255),
  ...
  "Ulteriori prescrizioni di sicurezza da adottare" character varying(255),
...
insert into temp_pdl select * from "Tab_001 Anagrafica_PdL.CAS";
...
delete from equipment where id_area in (select id_area from area_lavoro where id_impianto in (select id_impianto from impianti where id_azienda=(SELECT ID_AZIENDA from aziende where  codice ='CAS')));

delete from area_lavoro where id_impianto in (select id_impianto from impianti where id_azienda=(SELECT ID_AZIENDA from aziende where  codice ='CAS'));

delete from impianti where id_azienda=(SELECT ID_AZIENDA from aziende where  codice ='CAS');

-- impianti
insert into impianti (id_azienda,codice_impianto,descr_impianto,id_utente_ins)
select 
	(SELECT ID_AZIENDA from aziende where  codice ='CAS')
	,"Impianto"
...
SELECT 
	...
	,"Area di Lavoro"
	,(select id_utente from utenti where username='assistenza')
FROM "Tab_008 Lista Equipment.CAS" as tle 
left outer join impianti as imp on tle."Impianto"=imp.codice_impianto and imp.id_azienda in (select id_azienda from aziende where codice ='CAS') 
...
from (select distinct "Impianto","Area di Lavoro","Equipment","EIS"  from "Tab_008 Lista Equipment.CAS") as t1 
left outer join AREA_LAVORO as are on "Area di Lavoro"=descr_area and are.id_impianto in (select id_impianto from impianti where id_azienda in (select id_azienda from aziende where codice ='CAS') )
where "Equipment" is not null and are.id_area is not null
;

-- scarto queste equipment: 3
-- select * from "Tab_008 Lista Equipment.CAS" where "Equipment" is null or "Area di Lavoro" is null;


DELETE FROM PREPOSTI_IMPRESA WHERE ID_AZIENDA = (SELECT ID_AZIENDA from aziende where  codice ='CAS');

INSERT INTO PREPOSTI_IMPRESA (ID_AZIENDA,NOME_IMPRESA,NOME_COGNOME_PREPOSTO_IMPRESA)
SELECT 
DISTINCT
(SELECT ID_AZIENDA from aziende where  codice ='CAS') as ID_AZIENDA
,"Nome Azienda"
,"Riferimento Capo Cantiere" 
FROM "Tab_004 Ditte Terze.CAS"
...
select distinct 
	...
	,(select id_utente from utenti where username='assistenza')
from "Tab_001 Anagrafica_PdL.CAS" 
where "Impianto" not in (select codice_impianto from impianti where id_azienda in (select id_azienda from aziende where codice ='CAS'))
;	
...
select 
	...
	,true
	,(select id_utente from utenti where username='assistenza')
from "Tab_001 Anagrafica_PdL.CAS" as t1
left outer join impianti as imp on imp.codice_impianto = COALESCE(t1."Impianto",'ND') and imp.id_azienda in (select id_azienda from aziende where codice ='CAS')
...
alter table pdl alter column altro_sez1_prodotti1_testo type varchar;
alter table pdl alter column altro_sez1_attivita1_testo type varchar;


--SELECT * FROM  "Tab_001 Anagrafica_PdL.CAS"
--select * from PDL WHERE ID_AZIENDA=(SELECT ID_AZIENDA from aziende where  codice ='CAS') and substring(dt_pdl,1,4)='2018';
DELETE FROM PDL WHERE ID_AZIENDA=(SELECT ID_AZIENDA from aziende where  codice ='CAS');
--1899
...
SELECT 
	...
	are.id_area,
	equ.id_equipment,
	coalesce(to_char(coalesce(data_sez1,data_chiusura),'yyyy/mm/dd'),'2013/01/01') as DT_PDL, 
	...
	ALTRO_SEZ1_PRESCRIZIONI_SIC2_TESTO
FROM TEMP_PDL as p1
left outer join impianti as imp on  coalesce(p1.IMPIANTO,'ND') = imp.codice_impianto and imp.id_impianto in (select id_impianto from impianti where id_azienda in (select id_azienda from aziende where codice ='CAS') )
...
--select * from v_impianti where id_azienda=42;
