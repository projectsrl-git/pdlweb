-- Table: "Tab_001 Anagrafica_PdL.OSO"
-- select * from "Tab_001 Anagrafica_PdL.OSO";
...
insert into temp_pdl select * from "Tab_001 Anagrafica_PdL.OSO";
...
delete from equipment where id_area in (select id_area from area_lavoro where id_impianto in (select id_impianto from impianti where id_azienda=(SELECT ID_AZIENDA from aziende where  codice ='OSO')));

delete from area_lavoro where id_impianto in (select id_impianto from impianti where id_azienda=(SELECT ID_AZIENDA from aziende where  codice ='OSO'));

delete from impianti where id_azienda=(SELECT ID_AZIENDA from aziende where  codice ='OSO');

-- impianti
insert into impianti (id_azienda,codice_impianto,descr_impianto,id_utente_ins)
select 
	(SELECT ID_AZIENDA from aziende where  codice ='OSO')
	,"Impianto"
...
SELECT 
	...
	,"area di lavoro"
	,(select id_utente from utenti where username='assistenza')
FROM "Tab_008 Lista Equipment.OSO" as tle 
left outer join impianti as imp on tle."Impianto"=imp.codice_impianto and imp.id_azienda in (select id_azienda from aziende where codice ='OSO') 
...
from (select distinct "Impianto","area di lavoro","Equipment","EIS"  from "Tab_008 Lista Equipment.OSO") as t1 
left outer join AREA_LAVORO as are on "area di lavoro"=descr_area and are.id_impianto in (select id_impianto from impianti where id_azienda in (select id_azienda from aziende where codice ='OSO') )
where "Equipment" is not null and are.id_area is not null
;

-- scarto queste equipment: 3
-- select * from "Tab_008 Lista Equipment.OSO" where "Equipment" is null or "area di lavoro" is null;


DELETE FROM PREPOSTI_IMPRESA WHERE ID_AZIENDA = (SELECT ID_AZIENDA from aziende where  codice ='OSO');

INSERT INTO PREPOSTI_IMPRESA (ID_AZIENDA,NOME_IMPRESA,NOME_COGNOME_PREPOSTO_IMPRESA)
SELECT 
DISTINCT
(SELECT ID_AZIENDA from aziende where  codice ='OSO') as ID_AZIENDA
,"Nome Azienda"
,"Riferimento Capo Cantiere" 
FROM "Tab_004 Ditte Terze.OSO"
...
select distinct 
	...
	,(select id_utente from utenti where username='assistenza')
from "Tab_001 Anagrafica_PdL.OSO" 
where "Impianto" not in (select codice_impianto from impianti where id_azienda in (select id_azienda from aziende where codice ='OSO'))
;	
...
select 
	...
	,true
	,(select id_utente from utenti where username='assistenza')
from "Tab_001 Anagrafica_PdL.OSO" as t1
left outer join impianti as imp on imp.codice_impianto = COALESCE(t1."Impianto",'ND') and imp.id_azienda in (select id_azienda from aziende where codice ='OSO')
...
--SELECT * FROM  "Tab_001 Anagrafica_PdL.OSO"
--select * from PDL WHERE ID_AZIENDA=(SELECT ID_AZIENDA from aziende where  codice ='OSO') and substring(dt_pdl,1,4)='2018';
DELETE FROM PDL WHERE ID_AZIENDA=(SELECT ID_AZIENDA from aziende where  codice ='OSO');
--1899
...
SELECT 
	...
	ALTRO_SEZ1_PRESCRIZIONI_SIC2_TESTO
FROM TEMP_PDL as p1
left outer join impianti as imp on  coalesce(p1.IMPIANTO,'ND') = imp.codice_impianto and imp.id_impianto in (select id_impianto from impianti where id_azienda in (select id_azienda from aziende where codice ='OSO') )
