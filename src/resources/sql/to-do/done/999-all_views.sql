...
ON ACT.CODICE=DET.CODICE_TIPO_AZIONE AND CLA.DOMINIO = 'ACT'
...
FROM PDL
LEFT OUTER JOIN UTENTI ON UTENTI.ID_UTENTE_INS=ID_UTENTE
WHERE STATO='ACT'
group by id_azienda ,ID_IMPIANTO,ID_AREA,ID_PDL	,NR_PDL	,DESCRIZIONE_LAVORO	,dt_attivazione	,NOME_COGNOME_PREPOSTO_IMPRESA order by id_pdl
;
...
--vecchia query interferenze (colora solo rosso e arancio)
--SELECT 
--	PDL.* 
--	,PI.DESCR_AREA
--	,rect_x,rect_y,rect_h,rect_w,testo_x,testo_y,a_capo 
--	,rect_x_stampa,rect_y_stampa,rect_h_stampa,rect_w_stampa,testo_x_stampa,testo_y_stampa,a_capo_stampa
--	, case when (pdl.id_pdl not in (select id_pdl from limod70_dettagli left outer join limod70 on limod70.id_modulo=limod70_dettagli.id_modulo where stato='APP')) then '#ff3f3f' else '#ff9933' end as colore
--	, case when (pdl.id_pdl not in (select id_pdl from limod70_dettagli left outer join limod70 on limod70.id_modulo=limod70_dettagli.id_modulo where stato='APP')) then 'Da processare' else 'Processata' end as stato
...
DROP VIEW IF exists V_UTENTI_PROFILI_AZIENDE CASCADE;
...
-- view [V_IMPIANTI_ALL]
DROP VIEW IF exists V_IMPIANTI_ALL;

CREATE VIEW V_IMPIANTI_ALL
AS
SELECT 
	IMP.* 
	,AZI.RAGSOC AS DESCR_SITO
...
-- view [V_AREA_LAVORO_ALL]
DROP VIEW IF exists V_AREA_LAVORO_ALL;

CREATE VIEW V_AREA_LAVORO_ALL
AS
SELECT 
	ARE.*
	,imp.descr_impianto
...
-- view [V_EQUIPMENT_ALL]
DROP VIEW IF exists V_EQUIPMENT_ALL;

CREATE VIEW V_EQUIPMENT_ALL
AS
SELECT 
	EQU.*
	,are.descr_area
