...
group by v1.id_azienda,v1.id_impianto,al.ID_AREA,ID_PDL
	...
	,dt_attivazione
	,NOME_COGNOME_PREPOSTO_IMPRESA
	,NOME_IMPRESA,fl_tutto_impianto,id_pdl_corretto
order by al.id_area;
...
left outer join area_lavoro as al on pdl.id_area=al.id_area
WHERE STATO='ACT'
group by id_azienda ,pdl.ID_IMPIANTO,pdl.ID_AREA,ID_PDL	,NR_PDL	,DESCRIZIONE_LAVORO	,dt_attivazione	,NOME_COGNOME_PREPOSTO_IMPRESA ,fl_tutto_impianto,id_pdl_corretto


UNION

SELECT V_SIMULAZIONE_PDL_TUTTO_IMPIANTO.* FROM V_SIMULAZIONE_PDL_TUTTO_IMPIANTO
group by id_azienda ,ID_IMPIANTO,ID_AREA,ID_PDL	,NR_PDL	,DESCRIZIONE_LAVORO	,dt_attivazione	,NOME_COGNOME_PREPOSTO_IMPRESA,NOME_IMPRESA,V_SIMULAZIONE_PDL_TUTTO_IMPIANTO.EMAIL,fl_tutto_impianto,fl_tutto_impianto_simulato,id_pdl_corretto
order by id_area
;
...
SELECT distinct
	...
	,dt_pdl
	,odl
	,case when pdl.impresa_testo is null then vspti.nome_impresa else pdl.impresa_testo end as impresa_testo
...
case when fl_tutto_impianto_simulato and colore='red' then '' else colore end as colore from (
SELECT distinct
	PDL.* 
	,are.DESCR_AREA
...
CASE
  ...
  WHEN (stato='Da processare' and id_area = ANY( string_to_array((select lista_id_area from V_INTERFERENZE_TEMP_NEW_FULL), ',')::integer[] )) then 'red'
  ELSE ''
 END AS colore
 ,
